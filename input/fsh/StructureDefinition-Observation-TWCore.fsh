...
Alias: $vital-signs-tw = https://twcore.mohw.gov.tw/ig/twcore/ValueSet/vital-signs-tw

Profile: TWCoreObservation
Parent: Observation
Id: Observationtwcore
Title: "TW Core Observation"
...
* ^version = "0.0.1"
* ^date = "2022-10-13T11:33:23+08:00"
* ^contact.name = "衛生福利部"
...
* implicitRules ^comment = "宣告這套規則限制了內容只能被有限的貿易夥伴所理解。這從本質上限制了資料的長期有用性。然而，現有的健康生態體系高度分裂，還沒有準備好以普遍可計算的方式定義、收集和交換資料。只要有可能，實作者和／或規範編寫者應該避免使用這個資料項目。通常，在使用時，此URL是對實作指引的參照，此指引將這些特殊規則與其他profiles、value sets等一起定義為其敘述的一部分。"
...
* language ^example.valueCode = #zh-TW
* language ^binding.strength = #preferred
* language ^binding.description = "人類語言；鼓勵使用[CommonLanguages](http://hl7.org/fhir/R4/valueset-languages.html)代碼表中的代碼，但不強制一定要使用此代碼表，你也可使用其他代碼表的代碼或單純以文字表示。"
* text MS
...
* contained ^comment = "當內容可以從任何FHIR伺服器找到時，不應該以內嵌（contained）resource的方式呈現，因為一旦識別丟失，要再次恢復它是非常困難的（並且取決於情境）。內嵌（contained）的resource可在它們的meta資料項目中表述profiles和tag（標籤），但 **必須沒有（SHALL NOT）** 資訊安全標號（security labels）。"
...
* basedOn only Reference(CarePlan or DeviceRequest or ImmunizationRecommendation or $MedicationRequest-twcore or NutritionOrder or ServiceRequest)
...
* partOf only Reference(MedicationAdministration or MedicationDispense or MedicationStatement or $Procedure-twcore or Immunization or ImagingStudy)
...
* status ^binding.description = "檢驗檢查的狀態代碼；應填入[ObservationStatus](http://hl7.org/fhir/R4/valueset-observation-status.html)代碼表中的其中一個代碼。"
* category only $CodeableConcept-tw
* category ^short = "檢驗檢查分類"
...
* category ^binding.description = "高層次的檢驗檢查分類；鼓勵使用[ObservationCategoryCodes](http://hl7.org/fhir/R4/valueset-observation-category.html)代碼表中的代碼， 但不強制一定要使用此代碼表，你也可使用其他代碼表的代碼或單純以文字表示。"
* category.coding 1..
* category.coding ^definition = "由專門術語系統（terminology system）所定義之代碼的參照。"
* category.coding ^comment = "代碼可以在列舉清單（enumerations）或代碼清單（code lists）中非常隨意地定義，直至有非常正式的定義，如SNOMED CT—更多資訊見HL7 v3核心原則（Core Principles ）。編碼的排序是未定義的因而 **必須沒有（SHALL NOT）** 被用來推斷意義。一般來說，最多只有一個編碼值（coding values）會被標記為UserSelected = true。"
* category.coding ^requirements = "允許使用者在不知道識別碼系統的情況下利用識別碼"
* category.coding.id ^definition = "resource中資料項目的唯一ID（用於內部參照）。這可以是任何不含空格的字串。"
* category.coding.system 1..
* category.coding.system ^definition = "定義代碼中符號意義的代碼系統識別。"
* category.coding.code 1..
* code only $CodeableConcept-tw
* code from $vital-signs-tw (example)
...
* code ^binding.description = "應填入[生命體徵值集](https://twcore.mohw.gov.tw/ig/twcore/ValueSet-vital-signs-tw.html)代碼表中的其中一個代碼。"
* subject 1..
...
* performer only Reference($Practitioner-twcore or $Organization-twcore or $Patient-twcore or PractitionerRole or CareTeam)
* performer MS
* performer ^short = "誰負責這項檢驗檢查"
* performer ^definition = "誰負責宣告檢驗檢查結果值為「真的」"
* performer ^requirements = "可以對此檢驗檢查有一定程度的信任，並指出後續問題的方向。"
* value[x] MS
* value[x] from VitalSignsUnits (preferred)
...
* value[x] ^binding.description = "用於記錄生命體徵的常用UCUM單元；應填入[VitalSignsUnits](http://hl7.org/fhir/R4/valueset-ucum-vitals-common.html)代碼表中適合的代碼，確定無適合的代碼才可以使用其他代碼中的代碼來表示。"
...
* dataAbsentReason ^binding.description = "代碼具體說明檢驗檢查結果缺少的原因；應填入[DataAbsentReason](http://hl7.org/fhir/R4/valueset-data-absent-reason.html)代碼表中適合的代碼，確定無適合的代碼才可以使用其他代碼中的代碼來表示。"
* interpretation only $CodeableConcept-tw
...
* interpretation ^binding.description = "代碼指明檢驗檢查結果的解釋；應填入[ObservationInterpretationCodes](http://hl7.org/fhir/R4/valueset-observation-interpretation.html)代碼表中適合的代碼，確定無適合的代碼才可以使用其他代碼中的代碼來表示。"
...
* bodySite only $CodeableConcept-tw
...
* bodySite ^binding.strength = #example
* bodySite ^binding.description = "代碼說明解剖學位置，可能包含側邊；可參考[SNOMEDCTBodyStructures](http://hl7.org/fhir/R4/valueset-body-site.html)代碼表，但此代碼表只是針對這個欄位的一個可能值的示例，不預期也不鼓勵使用者一定要使用此代碼表的代碼。"
* method only $CodeableConcept-tw
...
* method ^binding.strength = #example
* method ^binding.description = "檢驗檢查的方法；可參考[ObservationMethods](http://hl7.org/fhir/R4/valueset-observation-methods.html)代碼表，但此代碼表只是針對這個欄位的一個可能值的示例，不預期也不鼓勵使用者一定要使用此代碼表的代碼。"
...
* referenceRange.type only $CodeableConcept-tw
...
* referenceRange.type ^binding.strength = #preferred
* referenceRange.type ^binding.description = "用以表述參考範圍值之含意的代碼；鼓勵使用[ObservationReferenceRangeMeaningCodes](http://hl7.org/fhir/R4/valueset-referencerange-meaning.html)代碼表中的代碼， 但不強制一定要使用此代碼表，你也可使用其他代碼表的代碼或單純以文字表示。"
* referenceRange.appliesTo only $CodeableConcept-tw
* referenceRange.appliesTo ^short = "參考範圍值的人群"
...
* referenceRange.appliesTo ^binding.strength = #example
* referenceRange.appliesTo ^binding.description = "用以指明參考範圍值適用的人群之代碼；可參考[ObservationReferenceRangeAppliesToCodes](http://hl7.org/fhir/R4/valueset-referencerange-appliesto.html)代碼表，但此代碼表只是針對這個欄位的一個可能值的示例，不預期也不鼓勵使用者一定要使用此代碼表的代碼代碼表中的代碼。"
...
* component.code MS
* component.code from $vital-signs-tw (example)
...
* component.code ^binding.description = "代碼指明檢驗檢查的名稱；可參考[生命體徵值集](https://twcore.mohw.gov.tw/ig/twcore/ValueSet-vital-signs-tw.html)代碼表，但此代碼表只是針對這個欄位的一個可能值的示例，不預期也不鼓勵使用者一定要使用此代碼表的代碼。"
* component.value[x] MS
* component.value[x] from VitalSignsUnits (preferred)
...
* component.value[x] ^binding.description = "用於記錄生命體徵的常用UCUM單元。；應填入[VitalSignsUnits](http://hl7.org/fhir/R4/valueset-ucum-vitals-common.html)代碼表中適合的代碼，確定無適合的代碼才可以使用其他代碼中的代碼來表示。"
* component.dataAbsentReason only $CodeableConcept-tw
...
* component.dataAbsentReason ^binding.description = "代碼指明為什麼結果（Observation.value[x]）缺少；應填入[DataAbsentReason](http://hl7.org/fhir/R4/valueset-data-absent-reason.html)代碼表中適合的代碼，確定無適合的代碼才可以使用其他代碼中的代碼來表示。"
* component.interpretation only $CodeableConcept-tw
...
* component.interpretation ^binding.description = "用以指明檢驗檢查的解釋之代碼。應填入[ObservationInterpretationCodes](http://hl7.org/fhir/R4/valueset-observation-interpretation.html)代碼表中適合的代碼，確定無適合的代碼才可以使用其他代碼中的代碼來表示。"
